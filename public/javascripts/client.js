$(document).ready(function() {
    ...
    $(".usertag-box").on("keyup", function(e) {
        ...
        usertag.length == 0 ? $(".usertag-preview").css("color",
            "#044B7F") : $(".usertag-preview").css("color",
            "#FFF");
    ...
    $(".deactivate-overlay").click(function() {
        ...
        $(".message.enjoy").on(
            ...
            function() {
                $(this).removeClass('animated bounceIn').delay(
                    500).fadeOut(2000, function() {
                    $(this).remove();
                    initClient(usertag);
                });
            });
    ...
    $(".about-container").hover(
        function() {
            $(".github-box").css('visibility', 'visible').slideDown(
                'fast');
        },
        function() {
            $(".github-box").css('visibility', 'visible').slideUp(
                'fast');
        }
    );
...
function initClient(usertag) {
    ...
    socket.on('new message', function(msgObj) {
        ...
        $(".message-bundle#" + randomId).css('left', msgObj.cssLeft.toString() +
            "%");
        $(".message-bundle#" + randomId + " .message").css('font-size',
            msgObj.cssFontSize.toString() + "rem");
        $(".message-bundle#" + randomId + " .usertag").css('font-size', (
            msgObj.cssFontSize * 0.6).toString() + "rem")
        $(".message-bundle#" + randomId + " .message").css('color',
            msgObj.cssColor[0]);
        $(".message-bundle#" + randomId + " .usertag").css('color',
            msgObj.cssColor[1]);
        $(".message-bundle#" + randomId + " .message").text(msgObj.msg);
        if (msgObj.usertag)
            $(".message-bundle#" + randomId + " .usertag").text("[" +
                msgObj.usertag + "]");
    ...
    socket.on('new topic', function(topic) {
        if (topic.url)
            $("span.chat-topic").html(
                "<em><span class='chat-topic'><a href='" + topic.url +
                "'>" + topic.title + "</a></span></em>");
        else
            $("span.chat-topic").html("<em><span class='chat-topic'>" +
                topic.title + "</span></em>");
    });
    ...
    $(".send-btn").click(function(e) {
        if (!formActive) {
            ...
            if ($(".message-prompt").val().length == 0 || $(
                    ".message-prompt").val().length > 100) {
                e.preventDefault();
            }
        }
    });
